...
installslowdns() {
    ...
    wget -P $DIR https://github.com/ZeynDev/D-Zeyn/raw/main/betadns/dns-server >/dev/null 2>&1  
...
initslow() {
    if ps x | grep -w dns-server | grep -v grep 1>/dev/null 2>/dev/null; then 
        ...
        tmx='curl -sO https://github.com/ZeynDev/D-Zeyn/raw/main/betadns/slowdns && chmod +x slowdns && ./slowdns'
